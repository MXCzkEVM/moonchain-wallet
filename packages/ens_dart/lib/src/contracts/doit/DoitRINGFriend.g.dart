...
class DoitRINGFriend extends _i1.GeneratedContract {
  DoitRINGFriend({
  ...
  Future<String> create(
    ...
    assert(checkSignature(function, 'bcebbe80'));
    final params = [
      tokenURI,
      signature,
    ];
  ...
  Future<BigInt> group(
    ...
    assert(checkSignature(function, '7b5cf44f'));
    final params = [user];
    final response = await read(
      function,
      params,
  ...
  Future<String> join(
    ...
    assert(checkSignature(function, '049878f3'));
  ...
  Future<String> leave({
    ...
    assert(checkSignature(function, 'd66d9e19'));
    final params = [];
    return write(
      credentials,
      transaction,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> userToGroup(
    _i1.EthereumAddress $param30, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[25];
    assert(checkSignature(function, 'f3b90296'));
  ...
  /// Returns a live stream of all GroupCreated events emitted by this contract.
  Stream<GroupCreated> groupCreatedEvents({
    _i1.BlockNum? fromBlock,
    _i1.BlockNum? toBlock,
  }) {
    final event = self.event('GroupCreated');
    final filter = _i1.FilterOptions.events(
      contract: self,
      event: event,
      fromBlock: fromBlock,
  ...
  /// Returns a live stream of all GroupJoined events emitted by this contract.
  Stream<GroupJoined> groupJoinedEvents({
    _i1.BlockNum? fromBlock,
    _i1.BlockNum? toBlock,
  }) {
    final event = self.event('GroupJoined');
    final filter = _i1.FilterOptions.events(
      contract: self,
      event: event,
      fromBlock: fromBlock,
  ...
  /// Returns a live stream of all GroupLeft events emitted by this contract.
  Stream<GroupLeft> groupLeftEvents({
    _i1.BlockNum? fromBlock,
    _i1.BlockNum? toBlock,
  }) {
    final event = self.event('GroupLeft');
    final filter = _i1.FilterOptions.events(
      contract: self,
      event: event,
      fromBlock: fromBlock,
...
class GroupCreated {
  GroupCreated(
    ...
    this.event,
        ...
        group = (response[1] as BigInt),
        tokenURI = (response[2] as String);
