...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  } else if ([@"getSystemBiometricsWithError" isEqualToString:call.method]) {
    [self getSystemBiometricsWithError:result];
...
- (void)getSystemBiometricsWithError: (void (^)(NSDictionary<NSString*, NSString *> *params))handler {
    LAContext *context = [[LAContext alloc] init];;
    NSError *authError = nil;
    
    NSMutableDictionary<NSString *, NSString *> *resultDict = [[NSMutableDictionary<NSString *, NSString *> alloc] init];
    
    NSString *errorKey = @"error";
    NSString *biometryTypeKey = @"biometryType";
    
    // To set biometric type in context
    [context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:&authError];
    
    if (authError != nil) {
        resultDict[errorKey] = [[self class] convertErrorCode:authError.code];
    }
    
    resultDict[biometryTypeKey] = [[self class] convertBiometricTypeCode:context.biometryType];
    
    handler(resultDict);
}

+ (NSString *)convertErrorCode: (LAError) errorCode {
    switch(errorCode) {
...
+ (NSString *)convertBiometricTypeCode: (LABiometryType) biometricTypeCode {
    switch(biometricTypeCode) {
