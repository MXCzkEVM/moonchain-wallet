      ...
      children: [
        ...
        Row(
          children: [
            ...
            if (MXCChains.isMXCChainsPlusEthereum(
                portfolioState.network!.chainId)) ...[
              const Spacer(),
              MxcChipButton(
                key: const Key('addTokenToPublicListButton'),
                onTap: () => portfolioPresenter.launchTokenListRepo(),
                title: translate('add_x').replaceFirst(
                  '{0}',
                  '${translate('token').toLowerCase()} ${translate('to').toLowerCase()} ${translate('public_list').toLowerCase()}',
                ),
                iconData: Icons.add,
                alignIconStart: true,
              )
